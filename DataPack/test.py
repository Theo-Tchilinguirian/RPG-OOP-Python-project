...
class Warrior(Player):
    ...
    def __init__(self, vig=80, bvg=80):
        Player.__init__(self, cls='Warrior', lvl=1, mon=0, hp=100, atk=35, dfs=35, chp='I: THE RUINS', klc=0, bhp=100,
                        kcm=5)
...
class Soldier(Player):
    ...
    def __init__(self, vig=90, bvg=90):
        Player.__init__(self, cls='Soldier', lvl=1, mon=0, hp=90, atk=40, dfs=60, chp='I: THE RUINS', klc=0, bhp=90,
                        kcm=5)
        self.set_stat_value('VIGOR', vig)
        self.set_stat_value('BASEVIGOR', bvg)
...
class Gladiator(Player):
    ...
    def __init__(self, vig=100, bvg=100):
        Player.__init__(self, cls='Gladiator', lvl=1, mon=0, hp=80, atk=35, dfs=35, chp='I: THE RUINS', klc=0, bhp=80,
                        kcm=5)
        self.set_stat_value('VIGOR', vig)
        self.set_stat_value('BASEVIGOR', bvg)
...
class Mage(Player):
    ...
    def __init__(self, mana=90, bmn=90):
        Player.__init__(self, cls='Mage', lvl=1, mon=0, hp=100, atk=25, dfs=35, chp='I: THE RUINS', klc=0, bhp=100,
                        kcm=5)
...
class Sorcerer(Player):
    ...
    def __init__(self, mana=80, bmn=80):
        Player.__init__(self, cls='Sorcerer', lvl=1, mon=0, hp=110, atk=30, dfs=30, chp='I: THE RUINS', klc=0, bhp=110,
                        kcm=5)
        self.set_stat_value('MANA', mana)
        self.set_stat_value('BASEMANA', bmn)
...
class Priest(Player):
    ...
    def __init__(self, mana=110, bmn=110):
        Player.__init__(self, cls='Priest', lvl=1, mon=0, hp=75, atk=20, dfs=25, chp='I: THE RUINS', klc=0, bhp=75,
                        kcm=5)
        self.set_stat_value('MANA', mana)
        self.set_stat_value('BASEMANA', bmn)
...
def ask_change_chapter(player_list, pseudonyme, player_entity):
    ...
    if player_current_level % 5 == 0 and player_current_chapter != 'IV: MISTYC STAGE':
        ...
        answer = check_answer_get_yes_no(
            "You are level {}. Do you want do go to the next chapter ?".format(player_current_level))
        if answer == 'yes':
...
def battle_check_player_first(player_list, pseudonyme, player_entity, current_ennemy, current_ennemy_type,
                              player_energy_class, player_attack_points_on_ennemy, player_energy_used,
                              current_ennemy_attack_points_on_player):
    while get_if_player_dead(player_entity) == False and get_set_is_ennemy_dead_and_add_player_money_if_it_is_dead(
            player_list, pseudonyme, current_ennemy) == False:

        player_attack_on_ennemy(player_entity, player_attack_points_on_ennemy, current_ennemy, player_energy_class,
                                player_energy_used)  # Prints 'You attack ennemy'
        print(
            "You used {} {} but regenerated 5 {}".format(player_energy_used, player_energy_class, player_energy_class))
        show_current_ennemy_stats(current_ennemy)

        if get_set_is_ennemy_dead_and_add_player_money_if_it_is_dead(player_list, pseudonyme,
                                                                     current_ennemy) == True and get_if_player_dead(
                player_entity) == False:
            ...
            print(
                "You died. your name will forever be forgotten. May you and your miserable adventure go to the void and never return.")
...
def battle_check_ennemy_first(player_list, pseudonyme, player_entity, current_ennemy, current_ennemy_type,
                              player_energy_class, player_attack_points_on_ennemy, player_energy_used,
                              current_ennemy_attack_points_on_player):
    while get_if_player_dead(player_entity) == False and get_set_is_ennemy_dead_and_add_player_money_if_it_is_dead(
            player_list, pseudonyme, current_ennemy) == False:
...
def battle(player_list, pseudonyme, current_ennemy):
    ...
    if who_attacks_first == 'PLAYER':
        ...
        is_ennemy_dead = battle_check_player_first(player_list, pseudonyme, player_entity, current_ennemy,
                                                   current_ennemy_type, player_energy_class,
                                                   player_attack_points_on_ennemy, player_energy_used,
                                                   current_ennemy_attack_points_on_player)
        print(is_ennemy_dead)
        ...
        is_ennemy_dead = battle_check_ennemy_first(player_list, pseudonyme, player_entity, current_ennemy,
                                                   current_ennemy_type, player_energy_class,
                                                   player_attack_points_on_ennemy, player_energy_used,
                                                   current_ennemy_attack_points_on_player)
...
def player_attack_on_ennemy(player_entity, player_attack_points_on_ennemy, current_ennemy, player_energy_class,
                            player_energy_used):
    current_ennemy_type = current_ennemy.get_ennemy_stats()['TYPE']
...
def login(player_list):
    ...
    if player_in_player_list(player_list, pseudonyme) == False:
        ...
        if len(player_list) > 0:
            ...
            answer = check_answer_get_yes_no(
                "Do you want to create a new player with the pseudonyme {} ?".format(pseudonyme))

            if answer == 'yes':
                password = input("""What is your password ?:
                        >>> """)

                player_list, pseudonyme, password, player_entity = player_entity_maker(player_list, pseudonyme,
                                                                                       password)
                return player_list, pseudonyme, password, player_entity
...
current_ennemy = main(player_list)
print(current_ennemy.get_ennemy_stats())
